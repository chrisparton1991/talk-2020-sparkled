...
enum Anchor { START = 0, CENTER = .5, END = 1}
...
class ElementBuilder {
  ...
  public async image(url: string, x: Placement, y: Placement, scale: number = 1) {
  ...
  public static calculatePlacementX(object: PIXI.Container, x: Placement) {
    ...
    return ElementBuilder.calculatePlacement(object.width, canvasWidth, safeMarginX, x) + (object.width / 2);
  }

  public static calculatePlacementY(object: PIXI.Container, y: Placement) {
    const {canvasHeight, safeMarginY} = getCurrentTheme();
    return ElementBuilder.calculatePlacement(object.height, canvasHeight, safeMarginY, y) + (object.height / 2);
  }

  private static calculatePlacement(objectSize: number, canvasSize: number, margin: number, placement: Placement) {
...
const fromStart = (offsetPx: number = 0): Placement => ({anchor: Anchor.START, offsetPx});
const fromCenter = (offsetPx: number = 0): Placement => ({anchor: Anchor.CENTER, offsetPx});
const fromEnd = (offsetPx: number = 0): Placement => ({anchor: Anchor.END, offsetPx});
