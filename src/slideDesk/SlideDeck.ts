...
class SlideDeck {
  ...
  public async loadGlobalObjects(loadFn: (slideDeck: SlideDeck) => GlobalObject<PIXI.DisplayObject>[]) {
  ...
  private drawCanvasBounds() {
    const {canvasWidth, canvasHeight, safeMarginX, safeMarginY} = getCurrentTheme();
